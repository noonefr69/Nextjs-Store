...
export default async function Products({ searchParams }) {
  ...
  const mensCloth = data.filter((item) => {
    return item.category == "men's clothing";
  });

  const sortedLowToHigh = [...mensCloth].sort((a, b) => a.price - b.price);
  const sortedHighToLow = [...mensCloth].sort((a, b) => b.price - a.price);
  const sortedByPopularity = [...mensCloth].sort(
    (a, b) => b.rating.rate - a.rating.rate
  );

  let displayData = mensCloth;
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent className={`w-[210px] mt-1 rounded`}>
                ...
                <DropdownMenuItem className={`m-0 p-0`}>
                  ...
                  <Link
                    className="text-zinc-700  duration-150 hover:bg-[#00000008] rounded py-1 px-2 cursor-pointer w-full h-full"
                    href="/products/men's-clothing?sort=popularity"
                  >
                    Sort by popularity
                  </Link>
                </DropdownMenuItem>
                <DropdownMenuItem className={`m-0 p-0`}>
                  ...
                  <Link
                    className="text-zinc-700  duration-150 hover:bg-[#00000008] rounded py-1 px-2 cursor-pointer w-full h-full"
                    href="/products/men's-clothing?sort=lowToHigh"
                  >
                    Sort by price: low to high
                  </Link>
                </DropdownMenuItem>
                <DropdownMenuItem className={`m-0 p-0`}>
                  ...
                  <Link
                    className="text-zinc-700  duration-150 hover:bg-[#00000008] rounded py-1 px-2 cursor-pointer w-full h-full"
                    href="/products/men's-clothing?sort=highToLow"
                  >
                    Sort by price: high to low
                  </Link>
                </DropdownMenuItem>
